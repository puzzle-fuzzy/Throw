...
- 根容器 `mx-auto w-full max-w-[400px]`，移动优先；消息流区域 `flex-1` 撑满剩余高度并内部滚动，输入区固定底部
- 桌面端两侧留白即可，不做装饰性背景元素
...
| 交互 | 行为 |
| --- | --- |
| 文本发送 | `TextArea` 自动增高（上限 5 行），Ctrl/⌘+Enter 或「发送」；发送后清空并保持焦点；8000 字上限接近时计数提示 |
| 附件选择 | 📎 `Button`(ghost) → 文件多选；>100MB 单文件或 >10 个直接 `Toast` 拒绝并列出超限项 |
...
- 触摸目标 ≥ 44px；文件名 truncate + Tooltip；320/390/600/1280 四档宽度检查文字溢出与布局
...
| 浏览器双端验证 | 两窗口真实 P2P 传输；模拟 ICE 失败走中转回退；断网重连；拖拽/粘贴发送；图片/视频预览；亮暗主题与四档宽度 |
