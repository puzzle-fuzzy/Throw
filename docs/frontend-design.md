...
- `connected` 表示双方已连接；P2P / 中转是**每个文件**各自的传输结果，不作为整间房的全局状态
- `peer-left` 是可恢复状态：保留消息时间线，以非阻断提示展示 5 分钟重连宽限；输入、选择文件、拖放和粘贴入口均关闭
- 只有 `closed` / `expired` 是终态，显示返回首页的出口；首页可创建新的临时房间

### 4.2 线框（connected 态）

```
┌────────────────────────────────────┐
│ [◈ 临时房间  ⓪①②③④⑤⑥] [对方已连接] ⏱ ⋮ 离开 │ ← 会话标题式顶栏（图标+房间码
├────────────────────────────────────┤    两行+复制；连接状态 Chip+倒计时靠后）
│          （ScrollShadow 消息流）      │
│ (⚖) ┌────────────────────┐          │
│ 头像 │ 文本气泡（浅灰底）     14:02   │ ← 对方：圆头像(32px,顶对齐,细描边)
│      └────────────────────┘          │   + bg-default 气泡 rounded-[20px]
│            ┌───────────────────┐     │
│            │ 文本气泡（accent底）14:02│ ← 自己：右对齐、无头像
│            └───────────────────┘     │   时间戳融入气泡右下角
│  ┌─ 文件卡片 · 对方发来 ──────────┐   │
...
| connected | 顶栏 `Chip` 显示「对方已连接」及空闲倒计时；空消息流提供「已连接，可以开始传输」和选择文件主操作 |
| 重连中 | 顶栏黄色状态 `Chip` 显示「正在重新连接」，输入区禁用，已收内容保留 |
| peer-left | 非阻断 `Alert`：提示对方可在 5 分钟宽限内用原会话重连，已收内容保留；不再开放输入或文件入口 |
| closed/expired | 覆盖层 `Card`：说明 +「返回首页」；从首页创建新房间 |
...
- **元信息**：文件类型 Lucide 图标（图片/视频/音频/压缩包/文档/通用）；文件名单行 truncate + 原生完整名称提示；人类可读大小；通道 `Chip`（P2P/中转）及状态 `Chip`
  - **图片**：完成后气泡内缩略图（本地 Blob，等比、固定比例容器），点击 `Modal` 放大，提供「下载」
  - **视频**：完成后内联原生 `<video controls>`（本地 Blob，`aspect-ratio` 固定容器）
- **传输中**：`ProgressCircle` + `ProgressBar`（含 aria）+ 实时速度 + 预计剩余 + 已传/总量；本端传完后显示「等待对方接收确认」；`取消`（danger ghost）双方均可触发
  - **失败**：错误原因 + `重试`（primary outline，自动换另一通道）+ `取消`
  - **已取消**：置灰收起为一行
- 系统提示行（居中小字）：「对方已加入」「房间将在 X 后过期」等
- 文件卡片同样带头像布局（对方文件在头像右侧，宽度自适应、上限 420px）
