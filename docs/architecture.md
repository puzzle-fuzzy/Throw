...
- **过期规则**：`waiting` 超过 2 小时无人加入 → 销毁；`active` 房间自最后一次**文件传输活动**（file-offer / 中转分片 / 下载 / 取消 / 回执）起 30 分钟空闲 → 销毁（前端顶栏倒计时展示，新传输重置）；成员断线超过宽限未重连即从房间移除，**双方均被移除后房间立即销毁**（隐私优先：文件尽早清理）；任何房间达到绝对寿命（24 小时）强制销毁。REST 创建/加入后一直未建立 WS 的占位成员，60 秒（join-grace）后释放名额。文本消息不计入传输活动
...
| transfer-idle | 30min | active 房间文件传输空闲倒计时（新传输重置，文本不重置） |
